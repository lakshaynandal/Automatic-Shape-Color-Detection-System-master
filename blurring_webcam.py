...
while True:
    ...
    im_gray = cv2.cvtColor(blurred, cv2.COLOR_BGR2GRAY)  #Converting to gray scale
    canny = cv2.Canny(im_gray, 80, 240, 3)
    _, thresh = cv2.threshold(canny, 185, 255, 0)
    ...
    for cnt in contours:
        ...
        if(M['m00'])!=0:
            ...
            elif len(approx) == 4 and area <= 60000.0:
